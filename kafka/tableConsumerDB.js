...
cron.schedule('* * * * * *', async () => {
    ...
    logger.info(`Старт tableConsumerDB`);
    ...
    try {
        ...
        if (result.rowCount !== 0) {
            for (row of result.rows) {
                ...
                await axios.post(`http://${config.SYSTEM.restConfig.etranService.host}:${config.SYSTEM.restConfig.etranService.port}/${config.SYSTEM.restConfig.etranService.name}`, row.imes_body_ext, configXml)
                    .then(async function (response) {
                        await parseString(response.data, async function (err, data) {
                            if (data.responseClaim.status[0] == 0) {
                                try {
                                    client = await pool.connect();
                                    var sql_imesEndPhase = `SELECT * from ${config.SYSTEM.dbFunctions.imesEndPhase} ($1,$2,$3,$4,$5,$6,$7,$8)`;
                                    result_imesEndPhase = await client.query(sql_imesEndPhase, [row.imes_id, 200, 0, "ok", true, false, data.responseClaim.transactions, 20]);
                                    await client.release();
                                ...
                                reg_info = `Ошибка. RC=${data.responseClaim.status[0]} при вызове ${config.SYSTEM.dbFunctions.imesGetNext}`;
